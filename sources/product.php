...
if ($id != '') {
    ...
    $Producthot = $d->rawQuery("select photo, name$lang,desc$lang, slugvi, slugen, sale_price, regular_price, discount, id from #_product where type = ? and find_in_set('hot',status) and find_in_set('hienthi',status) order by numb,id desc", array($type));
    /* Lấy tất cả sản phẩm */
    $where = "";
