        ...
        <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
                ...
                <div class="form-group-category row">
                    ...
                    <?php } ?>
                    <?php if (isset($config['newsletter'][$type]['tuvan']) && $config['newsletter'][$type]['tuvan'] == true) { ?>
                        <div class="form-group col-md-4">
                            <label for="tuvan">Tư vấn:</label>
                            <input type="text" class="form-control text-sm" name="data[tuvan]" id="tuvan" placeholder="Tư vấn" value="<?= (!empty($flash->has('tuvan'))) ? $flash->get('tuvan') : @$item['tuvan'] ?>" required>
                        </div>
                    <?php } ?>
                    ...
                    <?php if (isset($config['newsletter'][$type]['time']) && $config['newsletter'][$type]['time'] == true) { ?>
                        <div class="form-group col-md-4">
                            <label for="time">Ngày giờ đặt:</label>
                            <input type="text" class="form-control text-sm" name="data[time]" id="time" placeholder="Ngày giờ đặt" value="<?= (!empty($flash->has('time'))) ? $flash->get('time') : @$item['time'] ?>" required>
                        </div>
                    <?php } ?>
                    <?php if (isset($config['newsletter'][$type]['number']) && $config['newsletter'][$type]['number'] == true) { ?>
                        <div class="form-group col-md-4">
                            <label for="number">Số người:</label>
                            <input type="text" class="form-control text-sm" name="data[number]" id="number" placeholder="Số người" value="<?= (!empty($flash->has('number'))) ? $flash->get('number') : @$item['number'] ?>" required>
                        </div>
                    <?php } ?>
